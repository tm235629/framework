...
| S10 | **Post-send BCC feedback loop — built on MOT 2026-07-03, extraction pending (D5)** | MOT's `draft-outreach` skill gained a step 8: the sender BCCs themselves on the real send; the as-sent copy lands in the mail archive; the draft file is converted into a verbatim **SENT record** (`sent:` / `sent_source:` / `sent_cc:` frontmatter, agent draft archived with the collision counter); and the draft-vs-sent diff feeds a **dated, touch-type-scoped calibration entry** in the sender's voice profile (a "Live-send calibration log" section — n-counted directional rules that refine the corpus voice rules and are promoted into them only after recurring across ≥3 sends). `kb-contacts.mjs` already passes the optional draft `sent` field through (mirrored with MOT's extractor; exact-fidelity gate re-verified). Proven on send #1 (2026-07-03, dormant re-engage). Awaiting extraction once a few sends of evidence exist: the outreach-draft template's post-send step + a voice-profile-template calibration section. |
